...
class FFModel(nn.Module, BaseModel):
    ...
    def forward(
        ...
        next_obs_pred = obs_unnormalized + utils.unnormalize(delta_pred_normalized, delta_mean, delta_std)
    ...
    def get_prediction(self, obs, acs, data_statistics):
        ...
        # outputs.
        prediction, _ = self.forward(obs, acs, data_statistics['obs_mean'], 
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        ...
        target = utils.normalize(
            next_observations - observations, data_statistics['delta_mean'], data_statistics['delta_std'])
        # target = target.detach()?

        # loss = # TODO(Q1) compute the loss
        # Hint: `self(...)` returns a tuple, but you only need to use one of the
        # outputs.
        _, delta_pred_normalized = self.forward(observations, actions, data_statistics['obs_mean'],
                                                data_statistics['obs_std'], data_statistics['acs_mean'], data_statistics['acs_std'],
                                                data_statistics['delta_mean'], data_statistics['delta_std'])
